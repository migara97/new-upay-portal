...
<head>
    ...
    <meta name="description" content="@yield('meta_description', config('app.name', 'Laravel Portal') )">
    <meta name="author" content="@yield('meta_author', 'Seylan PLC')">
    <title>{{ config('app.name', 'SeylanPay Admin Portal') }}</title>
...
<div class="flex h-screen overflow-y-hidden bg-white dark:bg-slate-900" x-data="setup()"
    ...
    <div class="flex flex-col flex-1 h-full overflow-hidden" style="position: relative;">
        ...
        <x-backend.header/>
        <!-- Main content -->
        @if((!auth()->user()->is_ldap_user) && (!request()->is('admin/change-password')) && (password_change_notification_days()))
            <span class="text-danger error text-red-700 text-right mt-3 mr-4">Your password will expire with {{ password_change_remaining_days()  }} day. <a href="{{ route('admin.change-password') }}"><b>Change it</b></a> </span>
        @endif
