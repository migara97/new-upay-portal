<div class="grid grid-cols-1 gap-2">
    <div class="mb-4">
        <x-checkbox id="withOutLdap" label="Without AD Account" wire:model="withOutLdap"
                    wire:change="checkWithoutLDAP"/>
    </div>
    <x-input label="Email" placeholder="Email" wire:model.defer="email" wire:change="validateInput('email')" />

    @if(($this->allowLdap) || ($this->withOutLdap))
        <x-input label="Name" placeholder="Name" wire:model.defer="name" wire:change="validateInput('name')" :readonly="!$this->withOutLdap"/>
        @if($this->withOutLdap)
            <x-inputs.password label="Password" placeholder="Password" wire:model.defer="password"
                               wire:change="validateInput('password')"/>
            <x-inputs.password label="Confirm Password" placeholder="Confirm Password"
                               wire:model.defer="password_confirmation"
                               wire:change="validateInput('password_confirmation')"/>
        @endif
