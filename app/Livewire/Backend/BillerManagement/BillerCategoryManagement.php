    ...
    public function store(BillerCategoryRepositoryInterface $categoryRepository)
    {
        ...
        try {
            if ($this->checkCategoryExists()) {
                // activity_log($actionName, "A Category already exists with this details: Id -> $this->categoryId, Name -> $this->categoryName.");
            ...
            // activity_log($actionName, "Provider category [ $this->categoryName ] [ save ] successful.");
           ...
           activity_log($actionName, "Category create failed: Exception -> " . $exception->getMessage(), null, LogLevel::ERROR);
    ...
    public function update(BillerCategoryRepositoryInterface   $categoryRepository)
    {
        ...
        try {
            if ($this->checkOtherCategoryExists()) {
                // activity_log($actionName, "A Provider category already exists with this details: Category ID -> [ $this->categoryId ], Name -> [ $this->categoryName ].");
...
//
        //    activity_log($actionName, "Provider category [ $this->categoryName ] [ update ] successful.");
//            audit_log(AuditTrailAction::EDIT_BILLER_CATEGORY->name, "Provider category $this->categoryName, updated successfully.", json_encode($printData[0]), json_encode($printData[1]));
            ...
            // activity_log($actionName, "Provider category update failed: Exception -> " . $exception->getMessage() . " - " . $exception->getLine(), null, LogLevel::ERROR);
            $this->notification()->error(
                'Error !!!',
