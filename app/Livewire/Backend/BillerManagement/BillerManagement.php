...
class BillerManagement extends Component
{
    ...
    public function store(BillerRepositoryInterface $providerService)
    {
        ...
        try {
            if ($this->checkBillerExists()) {
                activity_log(ActionList::STORE_BILLER_PROVIDER->value, "A Provider already exists with this details: Provider Code -> [ $this->providerCode ], Name -> [ $this->providerName.]",PortalModules::BillerManagement);
            ...
            if($this->uploadFileName == null) {
                activity_log(ActionList::STORE_BILLER_PROVIDER->value, $uploadFileMessage, PortalModules::BillerManagement);
            ...
            activity_log(ActionList::STORE_BILLER_PROVIDER->value, 'Data : ' . $payload, PortalModules::BillerManagement);
            $providerService->storeProvider(json_decode($payload, true) ['data']);
            activity_log(ActionList::STORE_BILLER_PROVIDER->value, "Provider [ $this->providerName ] [ save ] successful.", PortalModules::BillerManagement);
            ...
            // activity_log(ActionList::STORE_BILLER_PROVIDER->value, "Provider create failed: Exception -> " . $exception->getMessage() . " - " . $exception->getLine(), null, LogLevel::ERROR);
    ...
    public function update()
    {
        ...
        try {
            ...
            // activity_log($actionName, 'Data : ' . $payload);
            ...
            // activity_log($actionName, "Provider [ $this->providerName ] [update] successful. ");
            ...
            // activity_log($actionName, "Provider update failed: Exception -> " . $exception->getMessage(), null, LogLevel::ERROR);
            $this->notification()->error(
                'Error !!!',
    ...
    public function updateProviderImageSave($id)
    {
        ...
        try {
            ...
            if ($this->imageUpdateUploadFileName == null) {
                // activity_log($actionName, $uploadFileMessage);
            ...
            // activity_log($actionName, "Provider [ $this->providerName ] [ update image ] successful.");
            ...
            // activity_log($actionName, "Provider image update failed: Exception -> " . $exception->getMessage() . " - " . $exception->getLine(), null, LogLevel::ERROR);
            $this->notification()->error(
                'Error !!!',
